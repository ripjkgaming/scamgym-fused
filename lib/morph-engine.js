...
export function initMorphEngine({ canvas, eyesSrc = "/assets/eyes.png", handsSrc = "/assets/hands.png", onProgress = null, onError = null } = {}) {
  ...
  let rigScale = 1;
  function fit() {
    // Fullscreen: the canvas is the viewport, so size from its own box and
    // scale the rig to fill the visible frustum on any aspect ratio.
    ...
    // Cover (not contain): fill the whole viewport on any aspect ratio.
    // Formed shapes may crop slightly on narrow screens; the dispersed
    // cloud always spans edge to edge.
    rigScale = Math.min(2.4, Math.max(visW / 5.2, visH / 4.0));
